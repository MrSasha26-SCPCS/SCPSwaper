...
function SCPSwaper:Update()
    if self.main.netEvent.isServer then
        ...
        if self.menu_time <= 0 then
            if self.scp_swap then
                self.scp_swap = false
                if #self.conn_list_prior > 0 then
                    local random = math.random(1, #self.conn_list_prior)
                ...
                elseif #self.conn_list > 0 then
                    local random = math.random(1, #self.conn_list)
